...
export default function Events() {
  ...
  return (
    <div className="h-auto w-screen flex items-center justify-center p-10 bg-[#F8F7FC]">
      <div className="flex-col space-y-8 w-full max-w-6xl">
        <h1 className="text-5xl text-[#BC1EB0] text-center mb-10 font-bold">
          EVENTS
        </h1>
        ...
        {currentEvents.map((event) => (
          ...
          >
            ...
            <div className="bg-[#BC1EB0] w-full h-1 md:w-1 md:h-auto" />

            <div className="flex-1 p-6 border-r border-gray-200">
              <div className="flex items-center mb-4 text-sm text-gray-500">
                <span className="w-2 h-2 bg-[#A21A9A] rounded-full mr-2"></span>
                {event.featured && (
                  <>
                    <span className="font-medium text-[#BC1EB0]">Featured</span>
          ...
          {Array.from({ length: totalPages }).map((_, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-full border transition-colors ${
                index === currentPage
                  ? "bg-[#BC1EB0] text-white border-[#BC1EB0]"
                  : "bg-white text-[#BC1EB0] border-[#A21A9A] hover:bg-[#F8F7FC]"
              }`}
            >
              {index + 1}
            </button>
